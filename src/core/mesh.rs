...
impl Mesh {
    ...
    pub fn is_3d(&self) -> bool {
        if self.fe_type == FEType::FE3D4 || self.fe_type == FEType::FE3D8 { true } else { false }
    }
    pub fn fe_volume(&self, i: usize) -> f64 {
        let x = self.get_fe_coord(i);
        match self.fe_type {
            FEType::FE1D2 => self.volume_1d2(x),
            FEType::FE2D3 => self.volume_2d3(x),
            FEType::FE2D4 => self.volume_2d4(x),
    ...
    pub fn volume_2d3(&self, x: Array2<f64>) -> f64 {
        // let a = ((x[[0, 0]] - x[[1, 0]]) * (x[[0, 0]] - x[[1, 0]]) + (x[[0, 1]] - x[[1, 1]]) * (x[[0, 1]] - x[[1, 1]]) + (x[[0, 2]] - x[[1, 2]]) * (x[[0, 2]] - x[[1, 2]])).sqrt();
        // let b = ((x[[0, 0]] - x[[2, 0]]) * (x[[0, 0]] - x[[2, 0]]) + (x[[0, 1]] - x[[2, 1]]) * (x[[0, 1]] - x[[2, 1]]) + (x[[0, 2]] - x[[2, 2]]) * (x[[0, 2]] - x[[2, 2]])).sqrt();
        // let c = ((x[[2, 0]] - x[[1, 0]]) * (x[[2, 0]] - x[[1, 0]]) + (x[[2, 1]] - x[[1, 1]]) * (x[[2, 1]] - x[[1, 1]]) + (x[[2, 2]] - x[[1, 2]]) * (x[[2, 2]] - x[[1, 2]])).sqrt();
    ...
    pub fn volume_2d4(&self, x: Array2<f64>) -> f64 {
        self.volume_2d3(array![ [x[[0, 0]], x[[0, 1]]], [x[[1, 0]], x[[1, 1]]], [x[[2, 0]], x[[2, 1]]] ]) + 
        self.volume_2d3(array![ [x[[2, 0]], x[[2, 1]]], [x[[3, 0]], x[[3, 1]]], [x[[0, 0]], x[[0, 1]]] ])
    }
    // Объем пирамиды
    ...
    pub fn be_volume(&self, i: usize) -> f64 {
        ...
        match self.fe_type {
            ...
            FEType::FE3D4 => self.volume_2d3(x),
            FEType::FE3D8 => self.volume_2d4(x),
        }
    }
