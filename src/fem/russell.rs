...
pub fn new_russell_solver(mesh: &Mesh) -> Result<GenericSolver<RussellSparseMatrix>, FemError> {
    let size = mesh.num_vertex * mesh.freedom;
    let nnz = 6 * mesh.nnz();
    let matrix = RussellSparseMatrix{size, data: SparseTriplet::new(size, size, nnz, Symmetry::No)?};
    Ok(GenericSolver::new(matrix, size))
}