...
impl<'a> ParamValue<'a> {
    ...
    fn new_fun(value: fn(f64, f64, f64) -> f64) -> Self {
        Self { value_type: ParamValueType::Function, value_str: "", value_fun: Some(value), value_vec: Vec::new() }
    }
...
impl<'a> Parameter<'a> {
    ...
    fn new_scalar_fun(p_type: ParamType, val: fn(f64, f64, f64) -> f64, pred: fn(f64, f64, f64) -> f64, direct: Direct) -> Self {
        let value = ParamValue::new_fun(val);
        let predicate = ParamValue::new_fun(pred);
        Self { p_type, value, predicate, direct }
    }
    fn new_vector_fun(p_type: ParamType, val: Vec<[f64; 2]>, pred: fn(f64, f64, f64) -> f64, direct: Direct) -> Self {
        let value = ParamValue::new_vec(val);
        let predicate = ParamValue::new_fun(pred);
        Self { p_type, value, predicate, direct }
    }
    pub fn get_scalar_value(&self, x: &ArrayView1<f64>, variables: &HashMap<&'a str, f64>) -> Result<f64, FemError> {
...
impl<'a> FEMParameter<'a> {
    ...
    pub fn add_stress_strain_curve_fun(&mut self, value: Vec<[f64; 2]>, predicate: fn(f64, f64, f64) -> f64) {
